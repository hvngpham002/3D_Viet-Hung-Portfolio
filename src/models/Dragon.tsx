...
type DragonProps = JSX.IntrinsicElements["group"];
...
const Dragon = (props: DragonProps) => {
  ...
  useFrame((_, delta) => {
    ...
    const zOffset = -0.9; // Distance away from camera
    ...
    const y =
      baseYOffset +
      waveAmplitude * Math.sin(clock.current * speed * waveFrequency);
    ...
    const nextZ =
      radiusZ * Math.cos((clock.current + delta * speed) * speed) + zOffset;
    const nextY =
      baseYOffset +
      waveAmplitude *
